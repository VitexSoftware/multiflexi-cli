...
class DataRetentionCleanup extends Command
{
    ...
    private function performCleanup(SymfonyStyle $io, InputInterface $input): int
    {
        ...
        self::displayCleanupSummary($io, $summary);
    ...
    private function generateReport(SymfonyStyle $io, InputInterface $input): int
    {
        ...
        // Display statistics
        self::displayStatistics($io, $cleanupStats, $archiveStats, $days);
        ...
        if ($outputFile) {
            ...
            switch ($format) {
                case 'json':
                    ...
                    break;
                case 'csv':
                    self::generateCsvReport($reportData, $outputFile);

                    break;

                default:
                    self::generateTextReport($reportData, $outputFile);
    ...
    private function showStatus(SymfonyStyle $io, InputInterface $input): int
    {
        ...
        $io->text(sprintf(
            'Cleanup jobs (last 7 days): %d completed, %d failed',
     ...
     */
    private static function displayCleanupSummary(SymfonyStyle $io, array $summary): void
    {
        $io->success(sprintf(
     ...
     * Display statistics.
     */
    private static function displayStatistics(SymfonyStyle $io, array $cleanupStats, array $archiveStats, int $days): void
    {
        ...
        $archiveTable = [
            ...
            ['Total size', self::formatBytes($archiveStats['total_size_bytes'])],
        ];
     ...
     * Generate CSV report.
     */
    private static function generateCsvReport(array $data, string $filePath): void
    {
     ...
     */
    private static function generateTextReport(array $data, string $filePath): void
    {
        $report = sprintf(
            ...
            self::formatBytes($data['archive_statistics']['total_size_bytes']),
        );
