 ...
 */
class TelemetryTestCommand extends MultiFlexiCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $enabled = Shared::cfg('OTEL_ENABLED', false);
        ...
        if ($endpoint = $input->getOption('endpoint')) {
            Shared::instanced()->configuration->setConfigValue('OTEL_EXPORTER_OTLP_ENDPOINT', $endpoint);
            $output->writeln('<comment>Using custom endpoint: '.$endpoint.'</comment>');
        }

        // Display current configuration
        $output->writeln('<info>Configuration:</info>');
        $output->writeln('  Service Name: '.Shared::cfg('OTEL_SERVICE_NAME', 'multiflexi'));
        $output->writeln('  Endpoint: '.Shared::cfg('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://localhost:4318'));
        $output->writeln('  Protocol: '.Shared::cfg('OTEL_EXPORTER_OTLP_PROTOCOL', 'http/json'));
        $output->writeln('');

        try {
            // Check if Telemetry class exists (from multiflexi-core)
            if (!class_exists('\\MultiFlexi\\Telemetry\\OtelMetricsExporter')) {
                $output->writeln('<error>MultiFlexi Telemetry support not found.</error>');
                $output->writeln('<comment>Please ensure you have the latest multiflexi-core package installed.</comment>');
            ...
            $exporter = new \MultiFlexi\Telemetry\OtelMetricsExporter();
