...
$CLI_CMD run-template:list
$CLI_CMD run-template:list --format json
$CLI_CMD run-template:list --limit 2
...
$CLI_CMD credential-prototype:list
$CLI_CMD credential-prototype:list --format json
$CLI_CMD credential-prototype:list --limit 2
$CLI_CMD credential-prototype:list --limit 1 --order D

# Create
$CLI_CMD credential-prototype:create --code TestCred123 --name "Test Credential" --uuid "12345678-1234-5678-9abc-123456789abc" --description "Test credential prototype"
$CLI_CMD credential-prototype:list

# Get
$CLI_CMD credential-prototype:get --code TestCred123
$CLI_CMD credential-prototype:get --uuid "12345678-1234-5678-9abc-123456789abc" --format json

# Update
$CLI_CMD credential-prototype:update --code TestCred123 --description "Updated test credential prototype"
$CLI_CMD credential-prototype:get --code TestCred123
...
if [ -f "/usr/lib/multiflexi/crprototype/subreg.crprototype.json" ]; then
    ...
    $CLI_CMD credential-prototype:import-json --file /usr/lib/multiflexi/crprototype/subreg.crprototype.json
    $CLI_CMD credential-prototype:import-json --file /usr/lib/multiflexi/crprototype/subreg.crprototype.json --format json
fi
...
# Queue overview
$CLI_CMD queue:overview
$CLI_CMD queue:overview --format json
...
echo "Testing run-template:list JSON format compliance"
RT_JSON=$($CLI_CMD run-template:list --format json)
...
echo "Testing credential-prototype:list JSON format compliance"
CRPROTO_JSON=$($CLI_CMD credential-prototype:list --format json)
...
echo "Testing credential-type:list JSON format compliance"
CREDTYPE_JSON=$($CLI_CMD credential-type:list --format json)
...
QUEUE_OV_JSON=$($CLI_CMD queue:overview --format json)
...
#TODO check config editing : $CLI_CMD run-template:update --id=159 --config=IMPORT_SCOPE=2025-08-01>2025-08-02 --format json
...
$CLI_CMD credential-prototype:delete --code TestCred123 --format json
$CLI_CMD credential-prototype:list
